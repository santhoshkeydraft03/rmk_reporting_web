...
const OtherExpensesInput = () => {
  ...
  const filteredExpensesData = useMemo(() => {
    ...
    return expensesData.filter(row => 
      row.expenseName?.toLowerCase().includes(query) ||
  ...
  const pinnedBottomRowData = useMemo(() => [{
    serialNo: 'Total',
    expenseName: `${filteredExpensesData.length} Records`,
    amount: filteredExpensesData.reduce((sum, row) => sum + (row.amount || 0), 0)
  }], [filteredExpensesData]);

  const previewPinnedBottomRowData = useMemo(() => previewData ? [{
    serialNo: 'Total',
    expenseName: `${previewData.length} Records`,
